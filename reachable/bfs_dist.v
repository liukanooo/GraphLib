...
Lemma path_of_len_valid_epath :
  forall u v d,
    path_of_len u v d ->
    exists p, valid_epath g u p v /\ length p = d.
Proof.
  intros u v d Hpath.
  induction Hpath as [u | u w v d _ [p [Hp Hlen]] Hstep].
  - exists nil.
    split; [apply valid_epath_empty | reflexivity].
...
Lemma valid_epath_path_of_len :
  forall u p v,
    valid_epath g u p v ->
    path_of_len u v (length p).
Proof.
  intros u p v Hvalid.
  revert u p v Hvalid.
  apply valid_epath_ind_n1.
  - intro v.
    simpl.
    constructor.
  - intros u p v w e _ IH Hstep.
    rewrite length_app.
    simpl.
    replace (length p + 1)%nat with (S (length p)) by lia.
    eapply pol_S.
    + exact IH.
    + exists e.
      exact Hstep.
...
Lemma is_path_valid_epath :
  forall (p: P) u v,
    is_path g p u v ->
    valid_epath g u (edge_in_path p) v.
Proof.
  intros p u v [Hpvalid [Hhead Htail]].
  exists p.
  split; [exact Hpvalid |].
  split; [reflexivity |].
  split; assumption.
...
Lemma path_weight_epath_weight :
  forall (p: P),
    path_weight g p = epath_weight g (edge_in_path p).
Proof.
  intro p.
  unfold path_weight, epath_weight, Zlist_sum.
  reflexivity.
Qed.

Lemma unit_epath_weight_length :
  forall
    (unit_weight: forall e u v, step_aux g e u v -> weight g e = Some 1%Z)
    u p v,
    valid_epath g u p v ->
    epath_weight g p = Some (Z.of_nat (length p)).
Proof.
  intros unit_weight u p v Hvalid.
  revert u p v Hvalid.
  apply valid_epath_ind_1n.
  - intro v.
    apply epath_weight_nil.
  - intros u v e p w Hstep _ IH.
    rewrite epath_weight_cons.
    rewrite (unit_weight e u v Hstep).
    rewrite IH.
    change (Some ((1 + Z.of_nat (length p))%Z) =
      Some (Z.of_nat (S (length p)))).
    f_equal.
    rewrite Nat2Z.inj_succ.
    lia.
Qed.

Lemma unit_path_weight_length :
  forall
    (unit_weight: forall e u v, step_aux g e u v -> weight g e = Some 1%Z)
    (p: P) u v,
    is_path g p u v ->
    path_weight g p = Some (Z.of_nat (length (edge_in_path p))).
Proof.
  intros unit_weight p u v Hpath.
  rewrite path_weight_epath_weight.
  eapply unit_epath_weight_length; [exact unit_weight |].
  apply is_path_valid_epath.
  exact Hpath.
Qed.

Theorem bfs_dist_iff_min_weight_distance_unit :
  forall v d,
    (forall e u w, step_aux g e u w -> weight g e = Some 1%Z) ->
    bfs_dist v d <->
    min_weight_distance g src v (Some (Z.of_nat d)).
Proof.
  intros v d unit_weight.
  split.
  - intros [Hpath Hmin].
    unfold min_weight_distance, min_value_of_subset_with_default.
    left.
    split.
    + unfold min_value_of_subset.
      destruct (path_of_len_valid_epath src v d Hpath) as [pe [Hpe Hlen]].
      destruct Hpe as [p [Hpvalid [Hedges [Hhead Htail]]]].
      assert (Hpispath: is_path g p src v).
      { unfold is_path.
        split; [exact Hpvalid | split; assumption]. }
      exists p.
      split.
      * unfold min_object_of_subset.
        split; [exact Hpispath |].
        intros q Hq.
        rewrite (unit_path_weight_length unit_weight p src v Hpispath).
        rewrite Hedges, Hlen.
        rewrite (unit_path_weight_length unit_weight q src v Hq).
        simpl.
        apply Nat2Z.inj_le.
        apply Hmin.
        apply valid_epath_path_of_len.
        apply is_path_valid_epath.
        exact Hq.
      * rewrite (unit_path_weight_length unit_weight p src v Hpispath).
        rewrite Hedges, Hlen.
    ...
    + simpl.
      exact I.
  - intro Hdist.
    unfold min_weight_distance, min_value_of_subset_with_default in Hdist.
    destruct Hdist as [[Hminvalue _] | [_ Hnone]].
    + unfold min_value_of_subset in Hminvalue.
      destruct Hminvalue as [p [Hobj Hpweight]].
      unfold min_object_of_subset in Hobj.
      destruct Hobj as [Hpispath Hpmin].
      pose proof (unit_path_weight_length unit_weight p src v Hpispath) as Hpweight_len.
      rewrite Hpweight_len in Hpweight.
      injection Hpweight as Hlen_eq.
      ...
      * rewrite <- Hlen_eq.
        ...
        exact Hpispath.
      * intros d' Hpath'.
        destruct (path_of_len_valid_epath src v d' Hpath') as
          [qe [Hqe Hqelen]].
        destruct Hqe as [q [Hqvalid [Hqedges [Hqhead Hqtail]]]].
        assert (Hqispath: is_path g q src v).
        { unfold is_path.
          split; [exact Hqvalid | split; assumption]. }
        specialize (Hpmin q Hqispath).
        rewrite Hpweight_len in Hpmin.
        rewrite (unit_path_weight_length unit_weight q src v Hqispath) in Hpmin.
        rewrite Hqedges, Hqelen in Hpmin.
        simpl in Hpmin.
        lia.
    + discriminate Hnone.
Qed.

End BFS_DIST_WEIGHTED.
